...
def main():
    ...
    for ep in range(EPOCHES):
        ...
        for images, labels in train_loader:
            ...
            # torch.eye(10)表明矩阵有10列 矩阵行数根据labels的长度决定但不会大于10
            labels = torch.eye(10).index_select(dim=0, index=labels).to(device)  #  batch_size， 10
            logits, reconstruction = model(images) # batch_size, out_capsules
    ...
    for images, labels in test_loader:
        ...
        logits, reconstructions = model(images) # logits shape = batch_size, out_capsules

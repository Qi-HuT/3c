...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, num_conv_units, in_channels, out_channels, kernel_size, stride):
        ...
        self.conv = nn.Conv2d(in_channels=in_channels,  # 256
                              out_channels=out_channels * num_conv_units,  # 8 * 32 = 256
    ...
    def forward(self, x):
        ...
        out = self.conv(x)  # batch 256 * 6 * 6
        # Flatten out: (batch_size, out_capsules * height * weight, out_channels)
        batch_size = out.shape[0]
        return squash(out.contiguous().view(batch_size, -1, self.out_channels), dim=-1)  # shape(5 1152 8)
...
class CapsuleLoss(nn.Module):
    ...
    def forward(self, images, labels, logits, reconstructions):
        ...
        # logits = torch.norm(out, dim=-1) logits已经是范数
        ...
        # torch matrix * matrix  当两者shape一样时为对应位置相乘， 当m1的shape为[3, 3] m2为[3, 1]时 是m2的每一行的数字乘m1的对应整行的每个数字
        # Reconstruction loss
